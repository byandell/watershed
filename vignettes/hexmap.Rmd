...
- **Zero-Overhead Code-Annealed HUC Scaling**:
  ...
  - Inspects 12-digit HUC string prefixes in memory (`HUC12` $\rightarrow$ `HUC10` $\rightarrow$ `HUC8` $\rightarrow$ `HUC6` $\rightarrow$ `HUC4`) after a single HUC12 spatial AOI query, selecting the finest level with $\le \text{max\_hucs}$ regions. Fetches parent geometries via a single direct ID query (`nhdplusTools::get_huc(id = target_ids, type = target_level)`), eliminating trial-and-error WFS network overhead.

- **Interactive Granularity Sidebar Option**:
...
- **Prompt Overhead Optimization**:
  - Streamlined [`AGENTS.md`](../AGENTS.md) developer guidelines to reduce prompt token overhead by ~83% while retaining all essential package build rules and quality standards.
